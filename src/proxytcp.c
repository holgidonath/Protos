...
new_connection(int client_fd)
{
    ...
    if (con != NULL)
    {
        ...
        buffer_init(con->read_buffer, N(CON-))
...
proxy_tcp_connection(struct selector_key *key){
    ...
    memcpy(&connection->client->client_addr, &client_addr, client_addr_len);
    connection->client->client_addr_len = client_addr_len;

    // Falta inicializar estructura de connection y setear stm

    if(SELECTOR_SUCCESS != selector_register(key->s, client, NULL, NULL, NULL)) {
        goto fail;
    }
